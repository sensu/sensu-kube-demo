...
kubectl create configmap sensu-enterprise-contacts --from-file=configmaps/sensu-enterprise/contacts/ --dry-run -o=yaml | kubectl replace -f -
...
kubectl patch deployment sensu-enterprise --patch "{\"spec\": {\"template\": {\"metadata\": {\"annotations\": {\"timestamp\": \"$(date +%s)\"}}}}}"
